...
class MyView(ui.View):
    ...
    def reflect_mainview(self):
        ...
        if self.direction_y == True:
            if self.y + self.height >= s_height:
...
def main():
    ...
    # サブビューの作成
    sub_view = MyView(frame=(0, 0, 150, 150))
    sub_view.background_color = 'blue'
    sub_view.speed = 1
    main_view.add_subview(sub_view)

    # サブビューの追加
    sub_view_2 = MyView(frame=(180, 180, 80, 80))
    sub_view_2.background_color = 'red'
    sub_view_2.speed = 2
    main_view.add_subview(sub_view_2)

    # サブビューの追加
    sub_view_3 = MyView(frame=(130, 280, 50, 50))
    sub_view_3.background_color = 'green'
    sub_view_3.speed = 3
    main_view.add_subview(sub_view_3)

    # サブビューの追加
    sub_view_4 = MyView(frame=(330, 400, 40, 40))
    sub_view_4.background_color = 'yellow'
    sub_view_4.speed = 2
    main_view.add_subview(sub_view_4)

    main_view.present()

    t = threading.Thread(target=schedule, args=[main_view])
    t.start()
