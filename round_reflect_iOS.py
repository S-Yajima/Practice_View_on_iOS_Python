#
# 動き回る円が、画面の端と、接触した円に反射します
# ToDo. 円同士の接触時の反射角がおかしいケースがあるのでフィックスする
#
...
class MyView(ui.View):
    ...
    def center_angle(self, diff_x, diff_y):
        ...
        if diff_x < 0 and diff_y > 0:
            center_angle = abs(center_angle) + 90
        ...
        for subview in self.superview.subviews:
            if id(self) != id(subview):
                ...
                if (self.width + subview.width) / 2 >= diff_center:
                    ...
                    self.angle = self.angle_with_wallangle((theta_center + diff_wall_angle) % 360)
                    self.calc_movement()
...
if __name__ == '__main__':
    ...
    main_view.name = '進む角度を設定した円を互いに反射'
